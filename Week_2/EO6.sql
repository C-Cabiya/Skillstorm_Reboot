...
CREATE PROCEDURE dbo.usfEX1
AS
	SET NOCOUNT ON;
	SELECT FORMAT(created_ts, 'yyyy') created_year, SUM(1)
	FROM users
...
CREATE PROCEDURE dbo.usfEX2
AS
	SELECT user_id, user_dob, user_email_id, DATENAME(WEEKDAY, user_dob)
	FROM users
...
SELECT user_id, UPPER(CONCAT(CONCAT(user_first_name, ' '), user_last_name)) user_name, 
	   user_email_id, created_ts, FORMAT(created_ts,'yyyy')
FROM users
...
SELECT user_unique_id, 
	CASE
	WHEN LEN(REPLACE(user_unique_id, '-', '')) = 9 THEN SUBSTRING(REPLACE(user_unique_id, '-', ''),6,4)
